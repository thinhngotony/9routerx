...
Sync targets:
  Claude Code   ~/.claude/settings.json    env.ANTHROPIC_BASE_URL (every run)
                env.ANTHROPIC_DEFAULT_*_MODEL only when unset/empty (setup);
                use --refresh-default-models to re-probe like before.
  Cursor        <Cursor settings.json>     openai.baseUrl
  Shell profile ~/.bashrc, ~/.zshrc        export ANTHROPIC_BASE_URL

Does not modify env.ANTHROPIC_AUTH_TOKEN (installer manages that once).
...
# Filled on first sync when keys are missing; cron leaves them alone unless
# --refresh-default-models is passed.
...
def sync_claude_code(
    ...
    Update ~/.claude/settings.json:
      env.ANTHROPIC_BASE_URL → <router_base>/v1 (always when it differs)
      env.ANTHROPIC_DEFAULT_*_MODEL → only if missing/empty, or always re-probed
      when refresh_default_models=True (legacy cron behaviour).
    ...
    if need_default_models:
        ...
        model_candidates = discover_model_candidates(
            router_base, api_key, use_combos=use_combos
        )
        ...
        print(
            "Claude Code: skipping ANTHROPIC_DEFAULT_*_MODEL probe "
            "(already set; use --refresh-default-models to re-pick)"
        )

    for env_key, candidates in model_candidates.items():
        current = env.get(env_key, "").strip()
        if not refresh_default_models and current:
            continue
        chosen = pick_working_model(
            router_base, candidates, current if refresh_default_models else "", api_key
        )
...
""",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Re-probe and update ANTHROPIC_DEFAULT_*_MODEL on every run. "
            "Default: only set those keys when missing (cron-safe)."
        ),
    )
    parser.add_argument(
        "--shell-profile",
